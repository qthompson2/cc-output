...
local function applySetCursorPos(t)
	...
	t.setCursorPos = function(x, y)
		if type(x) ~= "number" and type(x) ~= "nil" then
			error("Output.setCursorPos: bad argument #1 (number expected, got " .. type(x) .. ")")
		elseif type(y) ~= "number" and type(y) ~= "nil" then
			error("Output.setCursorPos: bad argument #2 (number expected, got " .. type(y) .. ")")
		end
...
local function applySetCursorBlink(t)
	...
	t.setCursorBlink = function(blink)
		if t.isSupported("setCursorBlink") then
			...
			error("Output.setCursorBlink: function not supported")
		end
	end
end
...
local function applyGetCursorBlink(t)
	...
	t.getCursorBlink = function()
		if t.isSupported("getCursorBlink") then
			...
			error("Output.getCursorBlink: function not supported")
		end
	end
end
...
local function applyIsColour(t)
	...
	local function isColour()
		if t.isSupported("isColour") then
			...
			error("Output.isColour: function not supported")
		end
	end
	t.isColour = isColour
	...
	local function setTextColour(colour)
		if type(colour) == "number" then
			if colour > 65535 or colour < 1 then
				error("Output.setTextColour: colour out of range (1 - 65535)")
			else
				t._current_fg = colours.toBlit(colour)
			end
		else
			error("Output.setTextColour: bad argument #1 (number expected, got " .. type(colour) .. ")")
		end
	end
...
local function applySetBackgroundColour(t)
	local function setBackgroundColour(colour)
		if type(colour) == "number" then
			if colour > 65535 or colour < 1 then
				error("Output.setBackgroundColour: colour out of range (1 - 65535)")
			else
				t._current_bg = colours.toBlit(colour)
			end
		else
			error("Output.setBackgroundColour: bad argument #1 (number expected, got " .. type(colour) .. ")")
		end
	end
...
local function applyGetPaletteColour(t)
	...
	local function getPaletteColour(index)
		if t.isSupported("getPaletteColour") then
			...
			error("Output.getPaletteColour: function not supported")
		end
	end
	...
	t.setTextScale = function(scale)
		if t._redirected then
			if t.isSupported("setTextScale") then
				...
				error("Output.setTextScale: function not supported")
			end
		else
			error("Output.setTextScale: function not supported on native")
		end
	end
end
	...
	t.getTextScale = function()
		if t._redirected then
			if t.isSupported("getTextScale") then
				...
				error("Output.getTextScale: function not supported")
			end
		else
			error("Output.getTextScale: function not supported on native")
		end
	end
end
...
local function applyBlit(t)
	...
	t.blit = function(text, fg, bg)
		...
		if type(text) ~= "string" then
			error("Output.blit: bad argument #1 (string expected, got " .. type(text) .. ")")
		elseif type(fg) ~= "string" then
			error("Output.blit: bad argument #2 (string expected, got " .. type(fg) .. ")")
		elseif type(bg) ~= "string" then
			error("Output.blit: bad argument #3 (string expected, got " .. type(bg) .. ")")
		end
		...
		for i = 1, #text do
			...
			if current_row[x + i - 1] ~= nil then
				if current_row[x + i - 1] ~= char
				or current_fg_row[x + i - 1] ~= fg[i]
				or current_bg_row[x + i - 1] ~= bg[i] then
					t["_modified"] = true
				end

				current_row[x + i - 1] = char
				current_fg_row[x + i - 1] = fg[i]
				current_bg_row[x + i - 1] = bg[i]
			end
		end
		t.setCursorPos(x + #text, y)
		if t._update_style == update_styles.on_modified then
...
local function applyScroll(t)
	...
	t.scroll = function(n)
		if type(n) ~= "number" then
			error("Output.scroll: bad argument #1 (number expected, got " .. type(n) .. ")")
		elseif n == 0 then
...
local function applyRedirect(t)
	...
	t.redirect = function(new_peripheral)
		...
		if type(new_peripheral) == "table" then
			...
			error("Output.redirect: bad argument #1 (table expected, got " .. type(new_peripheral) .. ")")
		end
	end
end
